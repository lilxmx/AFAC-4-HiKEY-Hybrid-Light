"""
Financial Contracts specialized retriever (BM25-only version).
...
Migrated from Golden_label_baseline. Uses BM25 only (no embedding).
"""
from typing import List, Dict

from methods._shared.indexer import BM25Index, Chunk
from .config import (
    BM25_TOP_K, MAX_EVIDENCE_CHARS
)
...
class FCRetriever:
    ...
    def _retrieve_for_doc(self, doc_id: str, queries: List[str],
        ...
        """
        Retrieve chunks for a single doc_id using BM25 with multi-query RRF fusion.
        ...
        # Collect BM25 rankings (one ranking per query)
        bm25_rankings = []
