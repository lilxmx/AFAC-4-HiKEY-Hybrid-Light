#!/usr/bin/env python3
"""Entry point for m05a_option_only (ablation: option-only retrieval)."""
import argparse
import logging
import sys
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"{METHOD_NAME}: HiKEY option-only retrieval (ablation)"
    )
    ...
    logger.info("Starting %s (ablation: option-only, no stem query)", METHOD_NAME)
    logger.info(
        "Model=%s coder=%s option_topk=%s final_topk=%s",
        config.MODEL_NAME,
        config.CODER,
        config.OPTION_TOPK,
    ...
    from pipeline import HiKEYOptionOnlyPipeline

    domains = args.domains or DOMAINS
    pipeline = HiKEYOptionOnlyPipeline()
