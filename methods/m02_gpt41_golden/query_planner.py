...
from config import (
    AZURE_API_KEY, AZURE_BASE_URL, REASONING_MODEL
)
...
输出格式（严格 JSON）：
{
      ...
      "claims": [
        {
          ...
          "target": "目标值",
          "requires_explicit_evidence": true/false
        }
      ],
      "logic": "AND/OR（多个claim时的聚合逻辑，默认AND）"
    }
  ],
  "field_packs": [
    {
  ...
  "final_logic": "AND/OR/INDEPENDENT（判断题用AND/OR，选择题用INDEPENDENT）"
}

注意事项：
- 对于选择题（mcq/multi），final_logic 设为 INDEPENDENT（每个选项独立判断）
...
class QueryPlanner:
    ...
    def plan(self, question: str, options: Dict[str, str],
        ...
        Returns:
            Structured retrieval plan dict
        """
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    ...
                    print(f"  [ERROR] Query Planner failed after {retries} attempts")
    ...
    def _build_user_prompt(self, question: str, options: Dict[str, str],
        ...
        """Build the user prompt for the Query Planner."""
        options_text = ""
...
请输出严格的 JSON 格式检索计划。"""
    ...
    def _validate_plan(self, plan: Dict, question: str, options: Dict[str, str],
        ...
        """Validate and fix the retrieval plan."""
        ...
        # Ensure all doc_ids in field_packs are valid
        ...
        # If no field_packs generated, create fallback
    ...
    def _fallback_plan(self, question: str, options: Dict[str, str],
        ...
        # Simple atomic claims: one per option
        atomic_claims = []
        for key, text in sorted(options.items()):
            claims = []
            for doc_id in doc_ids:
                if doc_id in text or f"第" in text:
                    claims.append({
                        "doc_id": doc_id,
                        "field": "general",
                        "operator": "verify",
    ...
    def _generate_fallback_field_packs(self, question: str, options: Dict[str, str],
        ...
        """Generate fallback field packs based on keyword detection."""
        # Combine all text for keyword detection
        all_text = question + " " + " ".join(options.values())

        # Detect relevant field packs
        field_packs = []
        for doc_id in doc_ids:
            keywords = []
            fields = []
