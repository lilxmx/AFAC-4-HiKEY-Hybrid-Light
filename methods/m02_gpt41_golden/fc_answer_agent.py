...
from config import (
    ...
    REASONING_MODEL, MAX_OUTPUT_TOKENS, VOTING_ROUNDS
)
...
逐项判断：
A. [分析] → supported/contradicted/insufficient → ✓/✗
B. [分析] → supported/contradicted/insufficient → ✓/✗
C. [分析] → supported/contradicted/insufficient → ✓/✗
D. [分析] → supported/contradicted/insufficient → ✓/✗
...
```
命题拆解：
P1: [子命题1] → 文档X: supported/contradicted/insufficient
P2: [子命题2] → 文档Y: supported/contradicted/insufficient

聚合逻辑：P1 AND P2
最终判断：全部 supported → A（正确）/ 任一非 supported → B（错误）
...
class FCAnswerAgent:
    """Financial Contracts Answer Agent with structured field verification."""
    ...
    def answer(self, question: str, options: Dict[str, str],
        ...
        Args:
            ...
            evidence_text: Formatted evidence string (organized by doc_id)
            answer_format: One of 'mcq', 'multi', 'tf'
            doc_ids: List of document IDs (order matters!)
            atomic_claims: Optional atomic claims from QueryPlanner
            voting_rounds: Number of voting rounds

        Returns:
            Dict with answer, confidence, reasoning, all_votes
        """
        ...
        for i, temp in enumerate(temperatures):
            ...
            if result:
                ...
                answer = self._normalize_answer(result['raw_answer'], answer_format)
    ...
    def _build_prompt(self, question: str, options: Dict[str, str],
        ...
        # Build analysis instruction based on format
        if answer_format == 'multi':
...
{analysis_instruction}"""
    ...
    def _call_api(self, prompt: str, temperature: float) -> Optional[Dict]:
        """Call GPT-4.1 API."""
        retries = 3
    ...
    def _multi_select_reflection(self, question: str, options: Dict[str, str],
                                  ...
                                  doc_ids: List[str],
                                  atomic_claims: List[Dict] = None) -> Optional[str]:
...
最后一行按字母顺序输出所有正确选项（如：ACD）。"""
    ...
    def _reflect(self, question: str, options: Dict[str, str],
                 ...
                 doc_ids: List[str],
                 atomic_claims: List[Dict] = None) -> Optional[str]:
        """General reflection for low-confidence answers."""
        options_text = ""
        for key in sorted(options.keys()):
            options_text += f"{key}. {options[key]}\n"
        ...
        for i, (ans, reasoning) in enumerate(zip(previous_answers, previous_reasoning)):
            if reasoning != "ERROR":
                snippet = reasoning[-600:] if len(reasoning) > 600 else reasoning
                prev_analysis += f"\n--- 分析{i+1}（答案：{ans}）---\n{snippet}\n"

        reflection_prompt = f"""以下是对同一道题目的多次分析，但得出了不同的答案。请你仔细审视这些分析，找出逻辑错误或遗漏。
...
1. 字段核验表是否正确
2. 三值判断是否准确
3. 是否混淆了本期/注册金额、主体/债项评级
4. "均/都" 是否正确执行了 AND 逻辑
5. 是否有 insufficient 被错误当成 supported

最后一行单独输出最终答案字母。"""

        result = self._call_api(reflection_prompt, temperature=0.1)
        if result:
            return self._normalize_answer(result['raw_answer'], answer_format)
        return None
    ...
    def _option_level_vote(self, answers: List[str]) -> Tuple[str, float]:
        ...
        return final_answer, avg_ratio

    def _majority_vote(self, answers: List[str]) -> Tuple[str, float]:
        """Majority voting for single-select and tf."""
        if not answers:
            return 'A', 0.0
        counter = Counter(answers)
