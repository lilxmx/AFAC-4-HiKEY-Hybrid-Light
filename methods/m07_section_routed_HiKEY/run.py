#!/usr/bin/env python3
"""Entry point for m07_section_routed_HiKEY."""
import argparse
import logging
import sys
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"{METHOD_NAME}: section-routed HiKEY retrieval"
    )
    ...
    parser.add_argument("--section-candidate-topk", type=int, default=None, help="Section candidate top-k")
    parser.add_argument("--routed-section-topm", type=int, default=None, help="Selected routed sections")
    ...
    if args.section_candidate_topk:
        config.SECTION_CANDIDATE_TOPK = args.section_candidate_topk
    if args.routed_section_topm:
        config.ROUTED_SECTION_TOPM = args.routed_section_topm
    ...
    logger.info(
        "Model=%s coder=%s final_topk=%s option_topk=%s section_topk=%s routed_topm=%s",
        config.MODEL_NAME,
        config.CODER,
        config.FINAL_EVIDENCE_TOPK,
        config.OPTION_TOPK,
        config.SECTION_CANDIDATE_TOPK,
        config.ROUTED_SECTION_TOPM,
    )

    from pipeline import HiKEYSectionRoutedPipeline

    domains = args.domains or DOMAINS
    pipeline = HiKEYSectionRoutedPipeline()
    ...
    summary = pipeline.save_results(extra_summary={
        "retrieval": {
            ...
            "section_candidate_topk": config.SECTION_CANDIDATE_TOPK,
            "routed_section_topm": config.ROUTED_SECTION_TOPM,
        }
    })
