"""
m05 pipeline: HiKEY retrieval with question+option local searches.

Compared with m04, this method does not ask the model with evidence retrieved
from the question only. It runs local retrieval for:
  - the question stem
  - question + option A
  - question + option B
  - question + option C
  - question + option D
Then it deduplicates all evidence items and sends only the best 20-30 items to
Qwen in one call.
"""
...
class HiKEYQuestionOptionIndexManager:
...
def _merge_evidence_packs(packs: List[Tuple[str, str, Dict[str, Any]]], final_topk: int) -> Dict[str, Any]:
    ...
    def final_score(item: Dict[str, Any]) -> float:
        # Repeated hits across question/options are good evidence, but do not let
        # popularity overwhelm the original retriever score.
...
class HiKEYQuestionOptionPipeline(BaseRunner):
    """Question+option retrieval pipeline."""
    ...
    def __init__(self):
        ...
        self.index_manager = HiKEYQuestionOptionIndexManager(HIKEY_INDEX_DIR)
    ...
    def _retrieve_question_option_evidence(
        ...
        if not evidence_text or evidence_text == "(未找到相关证据)":
            # Last-resort: search question+all options together.
